...
let [sign1, sign2, ...rest1] = ["양자리", "황소자리", "쌍둥이자리", "게자리", "사자자리"]; // 보통 끝에 적는다. rest 다른 이름으로 바꿔도 가능. 물론 예약어는 안 됨~! 에크마에서 계속 생기는 것
...
// 여기서는 rest!!! 나머지. 항상 뒤에 있어야...
let { productName: productName4, ...r } = pants;

console.log(`productName4 : ${productName4}`);
console.log(`r.color : ${r.color}`);
console.log(`r.price : ${r.price}`);
...
/* ... (3개의 점) spread-syntax(스프레드 문법, 전개 문법): 
...
// This function expects 2 arguments, but 3 were provided. [+2 locations]
...
/* ... rest일 때 이거는 뒤에 있어야 한다. 앞에 있으면 본인이 다 나머지인 것이다~
console. */
